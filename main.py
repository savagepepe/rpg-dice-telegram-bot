...
- Immagine risultato:
  ...
  - Numero del risultato PERFETTAMENTE CENTRATO nella sagoma (orizzontale e verticale)
...
def draw_fantasy_result(results: list[dict]) -> bytes:
    ...
    Genera un'immagine "scheda risultato":
      ...
      - risultato PERFETTAMENTE CENTRATO nella sagoma (orizzontale e verticale)
      - FONT: DEFAULT di Pillow
      - DIMENSIONE NUMERO: REGOLABILE (default 80)
    ...
    for res, label, cx in columns:
        ...
        # Calcolo preciso del bounding box per centrare perfettamente
        bbox = draw.textbbox((0, 0), totale_str, font=result_font)
        rw = bbox[2] - bbox[0]
        rh = bbox[3] - bbox[1]

        # Centro perfetto: cx, cy è il centro della sagoma
        x_num = cx - rw / 2
        y_num = cy - rh / 2
