...
- Supporto esplosioni (!)
- Immagine risultato:
  - Più piccola (circa 550x320)
  - Due colonne: sinistra = dado tiro, destra = dado fortuna (1d6)
  - Sagoma del dado in trasparenza
  - Numero del risultato AL CENTRO della sagoma, grande (≈ 80% della sagoma)
  - Font “antico” (Times/Georgia o simile), grassetto
  - Colore risultato:
      - rosso se il primo lancio è 1
      - verde se c'è¶¶ almeno un'esplosione
      - oro se neutro
  - Cornice dorata con stile medievale (doppio bordo)
...
def draw_fantasy_result(results: list[dict]) -> bytes:
    ...
    Genera un'immagine "scheda risultato":
      ...
      - risultato grande al centro della sagoma (≈ 80% della dimensione sagoma)
      - font “antico” (Times/Georgia), grassetto
      - colore risultato: rosso (primo lancio = 1), verde (esplosioni), oro (neutro)
      - cornice dorata con stile medievale (doppio bordo)
      - sotto, dettaglio lanci in piccolo
    """
    width, height = 550, 320
    bg_color = (12, 12, 20)  # molto scuro
    text_color = (220, 220, 220)
    oro_color = (220, 180, 60)
    rosso_color = (220, 60, 60)
    verde_color = (60, 200, 60)
    shape_alpha = 50  # trasparenza sagoma dado
    ...
    # Font “antico”
    try:
        title_font = ImageFont.truetype("Times New Roman Bold.ttf", 22)
        result_font = ImageFont.truetype("Times New Roman Bold.ttf", 1)  # ridimensionato dopo
        detail_font = ImageFont.truetype("Times New Roman.ttf", 16)
    except OSError:
        try:
            title_font = ImageFont.truetype("Georgia Bold.ttf", 22)
            result_font = ImageFont.truetype("Georgia Bold.ttf", 1)
            detail_font = ImageFont.truetype("Georgia.ttf", 16)
        except OSError:
    ...
    draw.text(((width - tw) / 2, 10), title, fill=text_color, font=title_font)
    ...
    for res, label, cx in columns:
        ...
        # Dimensione sagoma: circa 40% dell'altezza
        die_size = height * 0.45
        draw_die_shape(draw, res["faces"], cx, cy, die_size, shape_color)

        # Dimensione numero: ≈ 80% della sagoma
        number_size = int(die_size * 0.80)
        # Ricarico il font con la dimensione giusta
        try:
        ...
        rh = number_size  # approssimazione
        draw.text((cx - rw / 2, cy - rh / 2), totale_str, fill=result_color, font=result_font)

        # Etichetta sopra
        lw = draw.textlength(label, font=title_font)
        draw.text((cx - lw / 2, 45), label, fill=text_color, font=title_font)
