...
- Stile fantasy medievale, immagine compatta
- Macro: /d4, /d6, /d8, /d10, /d12, /d20
  Ogni macro lancia: 1dX! 1d6! (dadi separati, non sommati)
- Comando generico: /dadi <espressione>
- Supporto esplosioni (!)
- Immagine risultato:
  - Circa la metà delle versioni precedenti (~275x160)
  - Due colonne: sinistra = dado tiro, destra = dado fortuna (1d6)
  - Cornice dorata medievale (doppio bordo) COME PRIMA
  - Titolo "Risultato Tiri" in alto COME PRIMA
  - Etichette "Dado Tiro" / "Dado Fortuna" COME PRIMA
  - Sagoma del dado in trasparenza, colore solo leggermente più scuro dello sfondo
  - Sfondo in tonalità¾¾ seppia
  - Numero del risultato AL CENTRO della sagoma (orizzontale e verticale)
  - Numero molto grande (≈ 90-95% della sagoma)
  - Font “antico” e accattivante (Palatino/Times/Georgia), grassetto
  - Colore risultato:
      - rosso se il primo lancio è 1
      - verde se c'è¶¶ almeno un'esplosione
      - oro scuro/marrone scuro se neutro (in tema seppia)
  - Sotto, in piccolo, dettaglio lanci COME PRIMA
"""
...
def draw_fantasy_result(results: list[dict]) -> bytes:
    ...
    Genera un'immagine "scheda risultato":
      - circa 275x160 (metà±± rispetto a 550x320)
      - due colonne: sinistra = primo token (dado tiro), destra = secondo (fortuna)
      - cornice dorata medievale (doppio bordo) COME PRIMA
      - titolo "Risultato Tiri" in alto COME PRIMA
      - etichette "Dado Tiro" / "Dado Fortuna" COME PRIMA
      - sfondo seppia
      - sagoma dado colore solo leggermente più scuro dello sfondo
      - risultato grande al centro della sagoma (≈ 90-95% della sagoma)
      - font “antico” (Palatino/Times/Georgia), grassetto
      - colore risultato: rosso (primo lancio = 1), verde (esplosioni), oro scuro (neutro)
      - sotto, dettaglio lanci COME PRIMA
    """
    width, height = 275, 160
    # Sfondo seppia
    bg_color = (120, 100, 80)
    # Sagoma dado: colore solo leggermente più scuro
    shape_base = (90, 75, 60)
    shape_alpha = 90
    shape_color = shape_base + (shape_alpha,)

    # Colori testo e risultati
    text_color = (40, 30, 20)  # marrone molto scuro
    oro_color = (160, 130, 60)  # oro scuro, in tema seppia
    rosso_color = (180, 50, 50)
    verde_color = (50, 150, 50)
    neutro_color = (70, 50, 30)
    ...
    # Font “antico”
    try:
        title_font = ImageFont.truetype("Palatino Linotype Bold.ttf", 14)
        label_font = ImageFont.truetype("Palatino Linotype Bold.ttf", 12)
        detail_font = ImageFont.truetype("Palatino Linotype.ttf", 10)
    except OSError:
        try:
            title_font = ImageFont.truetype("Times New Roman Bold.ttf", 14)
            label_font = ImageFont.truetype("Times New Roman Bold.ttf", 12)
            detail_font = ImageFont.truetype("Times New Roman.ttf", 10)
        except OSError:
            try:
                title_font = ImageFont.truetype("Georgia Bold.ttf", 14)
                label_font = ImageFont.truetype("Georgia Bold.ttf", 12)
                detail_font = ImageFont.truetype("Georgia.ttf", 10)
            except OSError:
    ...
    inset = 6
    draw.rectangle([inset, inset, width - 1 - inset, height - 1 - inset], outline=border_color, width=1)

    # Titolo "Risultato Tiri" COME PRIMA
    title = "Risultato Tiri"
    tw = draw.textlength(title, font=title_font)
    draw.text(((width - tw) / 2, 4), title, fill=text_color, font=title_font)
    ...
    for res, label, cx in columns:
        ...
        # Sagoma dado
        die_size = height * 0.55
        draw_die_shape(draw, res["faces"], cx, cy, die_size, shape_color)

        # Numero: ≈ 90-95% della sagoma
        number_size = int(die_size * 0.92)
        try:
            result_font = ImageFont.truetype("Palatino Linotype Bold.ttf", number_size)
        except OSError:
            try:
                result_font = ImageFont.truetype("Times New Roman Bold.ttf", number_size)
            except OSError:
                try:
                    result_font = ImageFont.truetype("Georgia Bold.ttf", number_size)
                except OSError:
        ...
        # Etichetta "Dado Tiro" / "Dado Fortuna" COME PRIMA
        lw = draw.textlength(label, font=label_font)
        # Posizionata sopra il dado, come prima
        draw.text((cx - lw / 2, 20), label, fill=text_color, font=label_font)

        # Dettaglio lanci sotto COME PRIMA
        details_lines = []
        ...
        # Posiziono i dettagli sotto il dado, come prima
        y_detail = cy + die_size * 0.65
