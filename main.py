...
- Immagine risultato:
  ...
  - Sfondo NERO
  - Sagoma del dado in trasparenza, colore SEPPIA
  - Numero del risultato AL CENTRO della sagoma (orizzontale e verticale)
  - Numero dimensione ≈ 90% rispetto alla sagoma
...
def draw_fantasy_result(results: list[dict]) -> bytes:
    ...
    Genera un'immagine "scheda risultato":
      ...
      - etichette "Dado Tiro" / "Dado Fortuna" bilanciate sopra ogni dado
      - sfondo NERO
      - sagoma dado colore SEPPIA in trasparenza
    ...
    # Sfondo NERO
    bg_color = (10, 10, 12)
    # Sagoma dado: colore SEPPIA
    shape_base = (140, 110, 70)  # seppia caldo
    shape_alpha = 70
    shape_color = shape_base + (shape_alpha,)

    # Colori
    text_color = (210, 200, 180)  # chiaro, leggibile su nero
    oro_color = (180, 150, 60)    # oro per cornice e neutro
    rosso_color = (200, 60, 60)
    verde_color = (60, 180, 60)
    neutro_color = oro_color
    ...
    for res, label, cx in columns:
        ...
        # Sagoma dado (seppia)
        die_size = height * 0.52
        draw_die_shape(draw, res["faces"], cx, cy, die_size, shape_color)

        # Numero: 90% della sagoma
        number_size = int(die_size * 0.90)
        try:
        ...
        draw.text((cx - lw / 2, 22), label, fill=text_color, font=label_font)
