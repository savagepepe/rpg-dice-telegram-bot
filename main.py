...
- Immagine risultato:
  ...
  - Font numero interno alle sagome FISSO A 26 pt (+6 rispetto a 20)
  - Font “antico” (Palatino/Times/Georgia), grassetto
  - Tutte le altre scritte +5 pt rispetto alla versione precedente
  - Colore risultato:
...
def draw_fantasy_result(results: list[dict]) -> bytes:
    ...
    Genera un'immagine "scheda risultato":
      ...
      - FONT NUMERO INTERNO FISSO A 26 pt (+6 rispetto a 20)
      - ALTRE SCRITTE +5 pt rispetto alla versione precedente:
          title: 18 → 23
          label: 14 → 19
          detail: 12 → 17
      - font “antico” (Palatino/Times/Georgia), grassetto
    ...
    # Font “antico” con dimensioni aumentate
    try:
        title_font = ImageFont.truetype("Palatino Linotype Bold.ttf", 23)   # 18 → 23
        label_font = ImageFont.truetype("Palatino Linotype Bold.ttf", 19)  # 14 → 19
        detail_font = ImageFont.truetype("Palatino Linotype.ttf", 17)      # 12 → 17
        result_font_base = ImageFont.truetype("Palatino Linotype Bold.ttf", 26)  # 20 → 26
    except OSError:
        try:
            title_font = ImageFont.truetype("Times New Roman Bold.ttf", 23)
            label_font = ImageFont.truetype("Times New Roman Bold.ttf", 19)
            detail_font = ImageFont.truetype("Times New Roman.ttf", 17)
            result_font_base = ImageFont.truetype("Times New Roman Bold.ttf", 26)
        except OSError:
            try:
                title_font = ImageFont.truetype("Georgia Bold.ttf", 23)
                label_font = ImageFont.truetype("Georgia Bold.ttf", 19)
                detail_font = ImageFont.truetype("Georgia.ttf", 17)
                result_font_base = ImageFont.truetype("Georgia Bold.ttf", 26)
            except OSError:
                title_font = ImageFont.load_default()
                label_font = ImageFont.load_default()
                detail_font = ImageFont.load_default()
                result_font_base = ImageFont.load_default()
    ...
    for res, label, cx in columns:
        ...
        draw.text((cx - lw / 2, 28), label, fill=text_color, font=label_font)
