...
- Supporto esplosioni (!)
- Immagine risultato:
  - Circa la metà delle versioni precedenti (~280x170)
  ...
  - Font “antico” e accattivante (Palatino/Times/Georgia o simile), grassetto
  - Colore risultato:
      - rosso se il primo lancio è 1
      - verde se c'è¶¶ almeno un'esplosione
      - marrone scuro/oro scuro se neutro (in tema seppia)
...
def draw_fantasy_result(results: list[dict]) -> bytes:
    ...
    Genera un'immagine "scheda risultato":
      - circa 280x170 (metà±± rispetto a prima)
      ...
      - colore risultato: rosso (primo lancio = 1), verde (esplosioni), marrone scuro/oro scuro (neutro)
      - sotto, dettaglio lanci in piccolo
    """
    width, height = 280, 170
    # Sfondo seppia
    bg_color = (120, 100, 80)
    # Sagoma dado: colore leggermente più scuro
    shape_base = (90, 75, 60)
    shape_alpha = 90  # più visibile ma sempre morbido
    shape_color = shape_base + (shape_alpha,)

    text_color = (40, 30, 20)  # marrone molto scuro per testo
    rosso_color = (180, 50, 50)
    verde_color = (50, 150, 50)
    neutro_color = (70, 50, 30)  # marrone scuro per risultato neutro
    ...
    # Font “antico”
    try:
        title_font = ImageFont.truetype("Palatino Linotype Bold.ttf", 16)
        detail_font = ImageFont.truetype("Palatino Linotype.ttf", 12)
    except OSError:
        try:
            title_font = ImageFont.truetype("Times New Roman Bold.ttf", 16)
            detail_font = ImageFont.truetype("Times New Roman.ttf", 12)
        except OSError:
            try:
                title_font = ImageFont.truetype("Georgia Bold.ttf", 16)
                detail_font = ImageFont.truetype("Georgia.ttf", 12)
            except OSError:
    ...
    draw.text(((width - tw) / 2, 6), title, fill=text_color, font=title_font)

    # Colonne
    columns = []
    if len(results) >= 2:
        columns = [
            (results[0], "Tiro", width * 0.25),
            (results[1], "Fortuna", width * 0.75),
        ]
    elif len(results) == 1:
        columns = [
            (results[0], "Tiro", width * 0.25),
    ...
    for res, label, cx in columns:
        ...
        # Carico font “antico” in grassetto con questa dimensione
        try:
        ...
        # Per centrare bene, uso textbbox invece di textlength per avere anche l'altezza
        ...
        # Etichetta sopra (Tiro / Fortuna)
        lw = draw.textlength(label, font=title_font)
        draw.text((cx - lw / 2, 2), label, fill=text_color, font=title_font)

        # Dettaglio lanci sotto (molto compatto)
        details_lines = []
        for j, det in enumerate(res["dettagli"], start=1):
            ...
            line = f"[{rolls_str}]={det['subtotale']}{explode_mark}"
            details_lines.append(line)

        if res["mod"] != 0:
            mod_sign = "+" if res["mod"] > 0 else ""
            details_lines.append(f"Mod {mod_sign}{res['mod']}→{res['totale']}")
